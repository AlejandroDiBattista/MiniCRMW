...
export function ChatPanel({ client, messages, loading, status, onConnect, onBack, onDetails, detailsVisible, onMessage, className }: {
  client: Client | null
  messages: Message[]
  loading: boolean
    ...
    <section className={cn("h-full min-h-0 flex flex-col overflow-hidden bg-chat", className)}>
      ...
      {status.state !== "connected" ? (
        <Alert className="m-3 shrink-0 rounded-lg border-warning-border/30 bg-warning/10 text-warning-foreground">
          ...
          <AlertDescription className="flex items-center justify-between gap-3">
            <span>Conectá tu teléfono para enviar y recibir mensajes.</span>
            ...
            return (
              <div key={message.id} className={cn(index === messages.length - 1 && "crm-reveal")}>
                ...
                <div className={cn("flex", message.direction === "outgoing" ? "justify-end" : "justify-start")}>
                  <div className={cn(
                    "max-w-[86%] rounded-2xl px-4 py-3 text-[15px] leading-6 shadow-xs md:max-w-[74%]",
                    message.direction === "outgoing" ? "rounded-br-md bg-wa-bubble text-wa-bubble-foreground" : "rounded-bl-md border border-border/80 bg-card",
                  )}>
                    <p className="whitespace-pre-wrap break-words">{message.body}</p>
                    <div className={cn("mt-0.5 flex items-center justify-end gap-0.5 text-[10px] font-normal leading-none tabular-nums", message.direction === "outgoing" ? "text-wa-bubble-muted" : "text-muted-foreground")}>
                      <span className="opacity-60">{timeFormatter.format(new Date(message.timestamp))}</span>
                      {message.direction === "outgoing" ? <CheckCheck className={cn("size-3.5 stroke-[2.5] transition-colors duration-200", message.readAt ? "crm-check-in text-wa-read" : "text-muted-foreground/80")} /> : null}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="relative z-10 shrink-0 border-t border-border/80 bg-card px-2 py-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        <div className="flex w-full items-end gap-2">
          ...
          />
          <Button size="icon" className={cn("group/send size-11 shrink-0 rounded-xl shadow-xs transition-transform duration-150 active:scale-95", sentAcknowledgement && "bg-wa-green disabled:opacity-100")} onClick={send} disabled={!body.trim() || status.state !== "connected" || sending}>
